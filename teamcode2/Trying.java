...
public class Trying extends LinearOpMode {
    ...
    // Allowing a timer to start.
    public ElapsedTime runtime = new ElapsedTime();
    // Declare each of the 4 motors for the drivetrain.
    public DcMotor LFD;
    public DcMotor LBD;
    public DcMotor RFD;
    public DcMotor RBD;

    // Declare Arm Motor and servos
    public DcMotor armR;
    public Servo claw;  // Left claw
    public Servo claw2;  // Right claw
    
    // Declare the sensors
    private ColorSensor color;
    private DigitalChannel touch;
    
    // Declare the servos limits
    ...
    // Class to tell ALL motors to run to the set target position using encoders
    ...
    // Class to tell ONLY the arm motor to run to the set target position using encoders
    public void ArmRunToPosition() {
        armR.setMode(DcMotor.RunMode.RUN_TO_POSITION);
    }
    // Class to tell ONLY the drivetrain motors to run to the set target position using encoders
    ...
    // Class to tell ALL motors to reset their current encoder values to zero
    ...
    // Class to tell ONLY the drive motors to reset their current encoder values to zero
    ...
    // Class to tell ONLY the arm motor to reset it's current encoder value to zero
    ...
    // Class used to set the target position for the arm motor
    public void Arm(int Pos5) {
        armR.setTargetPosition(Pos5);
    }

    // Class used to display the arm motor's current position and power on the driver hub.
    ...
    // Class used to display the arm motor's current position on the driver hub.
    ...
    // Class used to apply a constant amount of power to the arm motor to stop the arm from falling down
    public void HoldPos(double Power) {
        armR.setPower(Power);
    }

    // Send calculated power ONLY to the arm motor
    ...
    // Send calculated velocity to the arm motor
    ...
    // Tell ALL motors to run without using encoders
    ...
    // Tell ALL motors to run using encoders
    ...
    // Tell the arm motor to run using encoders
    public void ArmRunUsingEncoders() {
        armR.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
    }

    // Tell the arm motor to run without using encoders
    public void ArmStopUsingEncoders() {
        armR.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
    }

    // Tell the drive motors to run using encoders
    ...
    // Tell the drive motors to run without using encoders
    ...
    // class used to tell if the touch sensor is being pressed
    public void Touch() {
    ...
    public void runOpMode() {
        // Declare where everything is located on the control/expansion hub
        ...
        // Set the directions which the motors move so the robot moves forward
        ...
        // Tell the drive motors to run without encoders
        LFD.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        LBD.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        RFD.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        RBD.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        ...
        //Put the claws in their home positions
        ...
        while (opModeIsActive()) {
            ...
            //Declare all of the limits of the servos
